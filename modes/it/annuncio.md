...
- `{###}` = prossimo numero sequenziale a 3 cifre, zero-padded. Allocarlo in modo atomico eseguendo `node core/reserve-report-num.mjs` (restituisce `{###}` su stdout), scrivere il report, poi eseguire `node core/reserve-report-num.mjs --release {###}` per liberare la prenotazione.
