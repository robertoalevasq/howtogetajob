...
```bash
node core/discover-ats.mjs --in companies.yml
```
...
```bash
node core/discover-ats.mjs --in companies.yml --write
```

Other forms:

```bash
node core/discover-ats.mjs Stripe Ramp Mollie          # names as positional args
node core/discover-ats.mjs --in companies.yml --summary # human-readable table
node core/discover-ats.mjs --in companies.yml --vendors gh,ashby  # restrict probes
node core/discover-ats.mjs --in companies.yml --vendors workday   # Workday only
```
