...
1b. **Premiere evaluation de chaque session :** Lancer `node core/cv-sync-check.mjs` via Bash. En cas d'alertes, prevenir le candidat
...
9. **Entrees tracker en TSV** -- NE JAMAIS editer applications.md directement pour de nouveaux ajouts. Ecrire le TSV dans `data/tracker-additions/`, `merge-tracker.mjs` gere la fusion
