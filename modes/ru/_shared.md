...
<!-- ============================================================
     ...
     ============================================================
     Персонализация → _profile.md и config/profile.yml.
...
| _profile.md | `_profile.md` | ВСЕГДА (пользовательские архетипы, нарратив, переговоры) |
...
1b. **Первая оценка в сессии:** Запустить `node core/cv-sync-check.mjs`. Если предупреждения — уведомить пользователя.
...
9. **Добавления в трекер как TSV** — не добавлять НОВЫЕ записи в `data/applications.md` напрямую; для новых записей писать TSV в `data/tracker-additions/` и делать merge. Прямое редактирование существующих строк допустимо для обновления `status`/`notes`.
