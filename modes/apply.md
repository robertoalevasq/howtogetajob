...
2. On a reply that reads as "done" (or a clear equivalent — "verified," "clicked it," "ready"): ask the candidate to paste the password back from the earlier message (the bot never stored it — see Step 5-alt item 7 above). Update the pending confirmation to `data: "awaiting password paste-back"` if this needs its own turn boundary (Telegram); interactively, this is just the next line of the same conversation.
