...
4. **Fill in batches, verify in batches — never snapshot after every single field, and never take a full-page `browser_snapshot` when a scoped one answers the question.** Fill a batch of fields (all of them, if the page has a handful; otherwise groups of roughly 4-5, or by the page's own logical sections) with `browser_click` / `browser_type` / `browser_select_option` / `browser_fill_form`, then verify every field in that batch registered correctly — the Workday React-field quirk in particular silently fails this check, so the verification itself is never skipped, only its scope and frequency shrink:
   ...
   - If a batch's verification finds any failure, narrow down within just that batch using `browser_find` per field to isolate which one broke, rather than assuming the whole batch is bad, re-verifying fields that already passed, or reaching for a full-page snapshot to do what a targeted lookup already answers. Once fields are filled and verified this way, the subagent returns:
   ```json
   {
...
**Before writing the section, reconcile `data/application-defaults.md`.** Step 6c #6 already says to cache a confirmed boilerplate-category answer the moment it's given — but that's a secondary instruction buried mid-loop with nothing forcing it, unlike this step's own hard "every completed run writes an Application Answers section" gate, and it was found live 2026-09-08 to be silently skipped in practice: a candidate confirmed Gender/Race-Ethnicity/Veteran-status/Disability-status answers across two separate applications in one session and neither ever reached the cache. Treat this step as the last unskippable checkpoint for it: before persisting the snapshot below, check every field this run answered against Step 6b's boilerplate categories (EEO/demographic self-identification, veteran/disability status, "how did you hear about us," "previously worked here," standard legal-acknowledgment checkboxes, electronic signature) and append any that are missing from `data/application-defaults.md` now, per Step 6b #3 — don't assume Step 6c already did it. Skip a field here if its true answer is inherently posting-specific rather than a fixed default (e.g. "how did you hear about us" answered as a named recruiter/referral tied to *this* company) — only cache what would legitimately be the same answer on the next, unrelated application.
