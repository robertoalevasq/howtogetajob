...
3. Spawn a subagent — pin `model` to `haiku` (Haiku 4.5) (this is mechanical execution, not evaluative judgment) — and give it:
...
4. **Fill in batches, verify in batches — never snapshot after every single field, and never take a full-page `browser_snapshot` when a scoped one answers the question.** Fill a batch of fields (all of them, if the page has a handful; otherwise groups of roughly 4-5, or by the page's own logical sections) with `browser_click` / `browser_type` / `browser_select_option` / `browser_fill_form`, then verify every field in that batch registered correctly — the Workday React-field quirk in particular silently fails this check, so the verification itself is never skipped, only its scope and frequency shrink:
   ...
   ```json
   {
     ...
     "resume_uploaded": {"label": "Resume/CV", "file_path": "..."},
   ...
   **`resume_uploaded` is mandatory whenever this application includes a resume/CV field, on both the success and failure path** — it must always carry the exact renamed copy's path from step 2, never the internal `output/*.pdf` path and never omitted. This closes a real gap (found live 2026-09-11, report #023 HD Supply): the subagent uploaded the resume successfully but the calling flow never captured what path it actually used, so Step 8's "Files used" section was written as "Not recorded" — an application had already gone to a real employer with no record of what filename they received. Two mechanical backstops now exist alongside this contract field, neither a substitute for actually filling it in: `core/hooks/guard-resume-filename.mjs` (a `PreToolUse` hook) blocks `browser_file_upload` outright if the path is the internal `output/{num}-...pdf` pattern, and `application-answers.mjs` throws rather than silently writing "Not recorded" if a Resume/CV-labeled `files` entry reaches Step 8 with no path. The calling flow must read `resume_uploaded.file_path` back from this contract and pass it straight into Step 8's `files` array — don't re-derive it, don't guess it, don't skip it because the upload "obviously" succeeded.
...
- Files used, including CV, cover letter, portfolio, or other uploads with version/path when known — for a resume uploaded via Step 7b, record both the canonical `output/*.pdf` path (our own reference) and the renamed filename actually presented to the ATS (e.g. "Roberto Vasquez - Salesforce.pdf"), even though the renamed copy itself is deleted from `.tmp/` right after upload — the record should reflect what a reviewer on the employer side actually sees, not just our internal filename. **The renamed filename comes from Step 7b's `resume_uploaded.file_path` — read it back from that contract, never re-derive or guess it** (see Step 7b item 4's full rationale); `application-answers.mjs`, called below, throws rather than silently writing "Not recorded" if this is missing.
...
6. **If this report has an entry in `data/.apply-browser-sessions.json`, stop the holder now: run `node core/apply-browser-holder.mjs --stop --report {num} --workspace {workspaceCwd}`.** The browser's only purpose was letting later turns of this application reconnect without re-authenticating; once the application is submitted there are no more turns to reconnect for. **Do not just send the recorded `pid` a plain termination signal yourself and assume the holder's own shutdown handler ran** — on Windows (this project's actual deployment platform) that either force-kills the process immediately or does nothing, so the handler never gets the chance; this was confirmed live across three real applications (#023, #028, #029, 2026-09-09) that each silently needed a hand-run `taskkill` plus a manually-edited state file before this command existed. `--stop` does both the kill and the state-file cleanup itself, correctly on either platform, so it's never left half-done.
