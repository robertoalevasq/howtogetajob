...
2. On a reply that reads as "done" (or a clear equivalent — "verified," "clicked it," "ready"): **first check `data/.apply-secrets.json` for this report's cached entry** (Step 5-alt item 7). If present and its `createdAt` is under 24 hours old, use the cached email/password directly — sign in via Playwright without asking the candidate for anything (skip straight to item 3 below). Only when the cache is missing or expired does this step fall back to the earlier behavior: ask the candidate to paste the password back from the original message. **Ask only — never restate, repeat, or confirm the password value yourself in this message**, even though it may still be visible earlier in this same conversation; the paste-back only means something as a check if the candidate is the one supplying the value. If they paste something that looks wrong, say so and ask them to re-check their saved copy rather than reminding them what you sent. Update the pending confirmation to `data: "awaiting password paste-back"` if this needs its own turn boundary (Telegram); interactively, this is just the next line of the same conversation.
...
4. **Fill in batches, verify in batches — never snapshot after every single field.** Fill a batch of fields (all of them, if the page has a handful; otherwise groups of roughly 4-5, or by the page's own logical sections) with `browser_click` / `browser_type` / `browser_select_option` / `browser_fill_form`, then take ONE `browser_snapshot` to verify every field in that batch registered correctly — the Workday React-field quirk in particular silently fails this check, so the verification itself is never skipped, only its frequency drops from once per field to once per batch. A full-page accessibility-tree snapshot is large and stays in the subagent's context for the rest of its turn; a snapshot-per-field pattern on a 5-6 field page meant 5-6 of them, found live 2026-08-31 to be a major driver of this flow's token cost. If a batch's verification finds any failure, narrow down within just that batch (re-snapshot after each of its fields to isolate which one broke) rather than assuming the whole batch is bad or re-verifying fields that already passed. Once fields are filled and verified this way, the subagent returns:
   ```json
   {
