...
```bash
node core/agent-inbox.mjs add "evaluate https://acme.com/jobs/42"
node core/agent-inbox.mjs add "draft a follow-up for application #7"
node core/agent-inbox.mjs add "run a scan and triage anything new"
```

## Inspect / resolve

```bash
node core/agent-inbox.mjs list            # pending items
node core/agent-inbox.mjs list --all      # include resolved items
node core/agent-inbox.mjs resolve 1 --result "scored 4.3 — report 012"
```
...
3. After each, mark it `[x]` and append `→ result: <one line>` — either by hand
   or with `node core/agent-inbox.mjs resolve <n> --result "..."`.
