...
<!-- ============================================================
     ...
     你的个性化配置应写入 _profile.md 中（绝不会被自动覆盖）。
...
| 个人定制策略 | `_profile.md` | 始终读取 (自定义画像偏好、求职叙事、谈判话术) |
...
检测到画像后，读取 `_profile.md` 中候选人针对该画像的特定表达框架与项目佐证。
...
2. 在每个会话的第一次评估前运行 `node core/cv-sync-check.mjs`。如果发现同步警告，提示候选人。
