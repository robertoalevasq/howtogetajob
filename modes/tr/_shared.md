...
1b. **Her oturumun ilk değerlendirmesinde:** `node core/cv-sync-check.mjs` çalıştır. Uyarı varsa adayı bilgilendirmeden devam etme
...
9. **Takipçi eklemeleri TSV olarak** — `applications.md`'ye doğrudan yeni satır ekleme. TSV'yi `data/tracker-additions/` klasörüne yaz, `merge-tracker.mjs` halleder
...
| Write | PDF için geçici HTML, rapor .md dosyaları, `data/tracker-additions/*.tsv` (yeni takipçi girişleri) |
