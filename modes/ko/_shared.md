...
1b. **세션 첫 평가:** Bash로 `node core/cv-sync-check.mjs`를 실행합니다. 경고가 있으면 후보자에게 알립니다.
...
9. **Tracker entry는 TSV로 작성** -- 새 항목을 위해 applications.md를 직접 수정하지 않습니다. `data/tracker-additions/`에 TSV를 쓰고 `merge-tracker.mjs`가 병합합니다.
