...
**NEVER SELF-REMEDIATE A CONFIGURATION WALL.** Confirmed live 2026-09-14: a real headless routing dispatch, when it hit `config/plugins.yml`'s `telegram.enabled: false`, fabricated a fake bot token into `.env` and flipped the flag to `true` on its own instead of reporting the block. Outside the explicit, human-supervised First-Time Setup below — which runs once, before any polling starts, with a real person walking through each step live — no step in this mode ever writes to `.env` or changes a plugin's `enabled` flag in `config/plugins.yml`. If a dispatch hits a wall because a plugin isn't enabled or a credential is missing, report the block plainly and stop; never invent credentials or self-enable a plugin to route around it.
...
1. **Cycle progress:** `node core/cycle-status.mjs --json` → branch on the **`liveness.state`** field it returns. **Never infer "running" from `step.id` alone** — a dead run's `step.id` stays frozen at whatever it was when the process died, which is exactly how a real `/status` reported a cycle as "running" for ~7 hours after it had crashed (found live 2026-08-27; before that fix `--json` carried no liveness field at all, so this step's own instruction told the agent to treat any non-`done` step as active).
   ...
   - `"paused"` → the run stopped **cleanly and on purpose** at a batch boundary (`lastStopReason: "batch-limit"`) or a session-limit cutoff (`"session-limit"`), released its lock, and is waiting for the daemon to dispatch the next batch automatically. This is normal, expected, and needs nothing from the candidate — do **not** report it as "running" (it isn't; the process has exited) and do **not** report it as dead or tell them to send `/run`. Say what it is, e.g. `⏸ Your search cycle finished batch {n} and is queued to continue automatically — {counters.pipelineUrlsPending} roles still to evaluate. Nothing needed from you; I'll message you when it's done.` For `"session-limit"` specifically, add when it resumes (`resumeNotBefore`). Added 2026-09-15 after a candidate sent `/status` 14 minutes after a batch boundary and was told "running" about an already-exited process, then `/status` again past the 40-minute window and was told "stalled" — two contradictory answers with no state change between them but the clock.
