...
[msg_id: X] stage: resume-approval|field-approval|submit-approval|batch-approval|question|edit-intent — <short description> — waiting since <date>
  report: NNN
  job_url: https://...
  data: <stage-specific JSON — swapped-bullet list + candidate JSON path for resume-approval, field_mapping for field-approval, filled-form summary for submit-approval, question text for question, eligible/excluded lists for batch-approval, empty for edit-intent>
...
- **`stage: submit-approval`, approve** — click Submit (the only point in this entire mode that does), then run `apply` mode's **Step 8** (persist the `## Application Answers` section) and **Step 9** (`set-status.mjs ... Applied`, `followup-seed.mjs`). Send confirmation: `✅ Applied to {Role} at {Company}.` Remove the pending confirmation, add a line to Recent Actions.
