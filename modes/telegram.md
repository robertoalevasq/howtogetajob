...
5. **On a reply to `stage: settings-edit`:** branch on `data.phase` — this is what distinguishes the candidate's raw answer to the field question (step 3, above) from their yes/correction reply to that answer's read-back:
   - **`phase: "awaiting-answer"`** (this reply is the raw answer to the question step 3 just asked): parse it per the field named in `data.field`, per the rules below, then send that field's read-back message and update the pending confirmation to `data: { field, phase: "awaiting-confirmation", pending: <parsed value(s)> }` — still `stage: settings-edit`, remain waiting. `pending` is the only place the parsed-but-unconfirmed value lives; nothing is written to `config/profile.yml` yet.
     ...
     - `targeting`: a reply naming roles stages a `target_roles.primary` update (`pending: { mode: "title_based", roles: [...] }`); a reply indicating a switch to industry-based asks the one follow-up `What industry, and do you know specific companies in it? (name a few, or I can suggest some)` first and stays in `phase: "awaiting-answer"` for that follow-up's reply before it has enough to read back (same "ask a single focused follow-up rather than guessing" latitude `telegram-onboarding.md` Step 4 uses) — once resolved, `pending: { mode: "industry_based", industry: { name, slug, description }, companies: [...] }`. Read-back once resolved: `Got it — targeting is now {summary}. Confirm?`
   ...
   - **`phase: "awaiting-confirmation"`** (this reply answers the read-back step above just sent): resolve exactly like `telegram-onboarding.md` Step 4b's own confirm-or-correct loop:
     - "yes"/equivalent: write `data.pending` into `config/profile.yml` (`location.city`/`timezone`/`country` for location; `location.work_mode` for work mode; `target_roles.primary` or `targeting_mode`+`target_industries` for targeting; `compensation.target_range`/`minimum` for salary; `location.visa_status`/`needs_sponsorship`/`authorized_in` for sponsorship — exactly per `telegram-onboarding.md` Step 4b's existing instructions for each of these fields). Then run the matching `_portals-pruning.md` step using the new value: location or work mode → its section 3 (`location_filter`); targeting roles added → sections 1-2 (`title_filter.positive` and `tracked_companies`/`search_queries` pruning); targeting switched to industry-based → section 4 (`industry_companies`, resolved via `discover` mode); targeting switched back to `title_based`, salary, or sponsorship → no `_portals-pruning.md` step applies. Re-send the Step 3h.1 menu (reflecting the new value), store a fresh `stage: settings-menu` pending confirmation, and wait.
