...
1b. **Første evaluering i hver session:** Kør `node core/cv-sync-check.mjs` via Bash. Ved advarsler, informér kandidaten
...
9. **Tracker-poster som TSV** -- rediger ALDRIG applications.md direkte for nye tilføjelser. Skriv TSV i `data/tracker-additions/`, `merge-tracker.mjs` håndterer sammenfletningen
