...
2. Store the parsed answers in `answers.roles`, `answers.primaryLocation`, `answers.otherLocations` (array, may be empty), `answers.workMode`, `answers.salary`, `answers.needsSponsorship` (boolean), and `answers.email`. `answers.workMode` must be one of `remote_only` / `remote_preferred` / `hybrid_ok` / `onsite_ok` — map the candidate's own words onto these (e.g. "remote only" → `remote_only`, "prefer remote but open to hybrid" → `remote_preferred`, "fine with hybrid" → `hybrid_ok`, "happy to be in office" → `onsite_ok`); if the reply doesn't clearly resolve to one of the four, ask a single focused follow-up rather than guessing — this feeds real evaluation scoring (`modes/oferta.md` Block A's Remote row), not just a form field. For `answers.email`: if the candidate's reply names one, use it verbatim; if question 6 went unanswered, extract the best candidate email found in the pasted CV (Step 3) instead — but either way, this is a *default*, not a final answer, until the candidate sees it in the read-back below and confirms it (found live 2026-08-29: an onboarding run silently wrote a CV-extracted university email — the address on an old resume, not the one the candidate actually wanted for job applications — straight into `candidate.email` with no read-back line ever showing it, so the mismatch surfaced only when the candidate hit an account-creation step during a live application days later; extracting a plausible-looking value from the CV is exactly as unsafe here as it is for the GitHub-guessing bug below, for the same reason — a document can contain a real fact that still isn't the one the candidate wants used). This is the source Step 4b writes from, so nothing needs re-parsing after confirmation. **Never infer `answers.needsSponsorship` from location alone** (found live 2026-08-25: an onboarding run derived it from "Tampa, FL" and silently inherited the seeded template's "no sponsorship needed, US-authorized" default instead — living somewhere doesn't establish work-authorization status; a candidate could live in the US on a visa needing sponsorship, or vice versa). If the reply doesn't clearly answer this specific question, ask it again as its own focused follow-up before proceeding — this is exactly the kind of fact AGENTS.md's Source-of-Truth Boundary requires to come from an explicit statement, not an assumption, since it feeds real evaluation scoring (work-authorization check) and real visa-sponsorship answers on real job applications.
...
2. Once confirmed ("yes" or equivalent):
   ...
   e. Send: `One more optional round — three quick ones, all skippable:\n1️⃣ How would you pitch yourself professionally, and what's your #1 strength?\n2️⃣ Any deal-breakers? (e.g. "no on-site," "no startups under 20 people," "no Java shops")\n3️⃣ Any published articles, case studies, or open-source projects?\n\nReply with any/all of these. Reply "skip" to finish now — you can always tell me more later.`
   f. Advance `currentStep` to `profile_narrative`, save state.

## Step 4c — Headline, deal-breakers, proof points (optional)

On receiving the `profile_narrative` reply:

1. If it's "skip" (or equivalent): leave `workspaces/{slug}/config/profile.yml`'s `narrative.headline` (`""`) and `narrative.superpowers` (`[]`) as Step 4b left them, and leave `workspaces/{slug}/_brief.md`'s "Hard DQ Criteria" section as Step 4b's "none yet" — this is still correct, not a gap, exactly like the Discord webhook being skippable.
2. Otherwise, this reply may answer one, two, or all three questions — best-effort natural-language extraction per question, no follow-up asked for any of them (unlike Step 4's mandatory parsing, this whole round is optional, so an ambiguous or unanswered sub-question is just left blank rather than blocking on a clarifying question):
   - **Pitch/strength** → write to `narrative.headline`/`narrative.superpowers` in `profile.yml`, **lightly polished, not verbatim and not embellished**: fix grammar/phrasing/conciseness for a professional tone, but preserve every factual claim exactly as given and never add a claim, metric, or descriptor the candidate didn't state — the same "keywords get reformulated, never fabricated" discipline `core/AGENTS.md`'s Source-of-Truth Boundary already requires everywhere else in this system (CV tailoring, cover letters), applied here to the candidate's own self-description instead of CV bullets. A single strength becomes a one-item `narrative.superpowers` list; multiple strengths in one reply split into separate list items, each polished the same way. The "how would you pitch yourself" half of the reply goes to `narrative.headline`.
   - **Deal-breakers** → replace `workspaces/{slug}/_brief.md`'s "Hard DQ Criteria — instant FAIL" placeholder bullets with one bullet per stated deal-breaker, in the candidate's own words, lightly polished for clarity only (same non-fabrication rule as above) — never invent a deal-breaker they didn't state, and never leave the section's example placeholders in place alongside real ones. This is what `modes/triage.md`'s first-pass filtering reads to instant-fail postings that structurally can't work for this candidate; skipping this question is why that section otherwise stays empty forever.
   - **Published work** → append one proof-point bullet per item (article, case study, open-source project) to `workspaces/{slug}/_brief.md`'s "Proof Points" section **and** `profile.yml`'s `narrative.proof_points` list (`{name, url, hero_metric}` shape — omit `url`/`hero_metric` if the candidate didn't give one), alongside (never replacing) the CV-derived entries Step 4b already wrote. Never invent a metric or scope the candidate didn't state.
