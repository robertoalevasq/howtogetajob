...
<!-- ============================================================
     ...
     你的個人化設定應寫入 _profile.md 中（絕不會被自動覆蓋）。
...
| 個人客製策略 | `_profile.md` | 一律讀取（自訂原型偏好、求職敘事、談判話術）|
...
偵測到原型後，讀取 `_profile.md` 中求職者針對該原型的特定表達框架與專案佐證。
...
2. 在每個會談的第一次評估前執行 `node core/cv-sync-check.mjs`。若出現同步警告，提示求職者。
