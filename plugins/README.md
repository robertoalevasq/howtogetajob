...
2. **Provide its keys** in your own `.env` (each plugin declares which it needs).
   Run `node core/doctor.mjs` or `node core/plugins.mjs list` to see what's missing.
...
```bash
node core/plugins.mjs list
node core/plugins.mjs run gmail                       # ingest
node core/plugins.mjs run notion search "platform"    # search
node core/plugins.mjs run notion export [--dry-run]   # export
```
...
- **Centralized infrastructure** the project would operate — hosted job
  ...
  opt-in service**, not the open-core.
