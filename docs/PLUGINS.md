...
```bash
node core/plugins.mjs list          # what's installed + its trust badge
node core/plugins.mjs available     # bundled + community plugins we've approved
node core/plugins.mjs add <name>    # install an approved community plugin
node core/plugins.mjs enable <id>   # show the capability card (then add --confirm)
node core/plugins.mjs skill <id>    # print a plugin's how-to (if it ships one)
```

Two gates must both be satisfied for a plugin to run: it must be **enabled**
(`node core/plugins.mjs enable <id> --confirm`, which records your consent) **and** its
keys must be in your `.env`. `node core/doctor.mjs` shows what's missing.
...
asks you to review + `node core/plugins.mjs trust <id>` to re-pin (tamper detection).

## Writing a plugin

```bash
node core/plugins.mjs new my-plugin     # scaffolds plugins.local/my-plugin/
```
...
3. Open a **registry PR** (the `?template=plugin-registry.md` template — the
   ...
   `node core/plugins.mjs add <name>` and your plugin ships to them via the normal
   update.
...
3. Once approved + pinned, anyone who runs `node core/plugins.mjs add career-ops-plugin-<id>`
   installs your version, and the engine gives **your maintained successor
   precedence over the bundled reference** of the same id. `node core/plugins.mjs available`
...
- **Centralized infrastructure** the project would run (hosted aggregation,
  shared services, proxies) → out of scope; would need to be a separate,
  opt-in service.
