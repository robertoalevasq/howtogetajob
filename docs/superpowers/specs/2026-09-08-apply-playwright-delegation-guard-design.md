...
- **The hook's command (`node core/hooks/guard-playwright-delegation.mjs`) is a relative path, so it depends on the invoking process's cwd being the repo/workspace root.** If something ever invokes Claude Code with a different cwd, `node` fails to find the script and the hook fails open (allow) silently — no error surfaced to the user, just quietly no longer blocking. Not a currently known problem: every real invocation path (`telegram-monitor.mjs` spawns `claude -p` with `cwd` set to the workspace dir) already satisfies this. Noted here so the assumption is written down rather than implicit.
