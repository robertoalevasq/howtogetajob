...
- **This drift class has no existing detection, and that's a confirmed gap, not a hypothetical one.** `.claude/settings.json` is provisioned via `SYSTEM_FILE_COPIES` — a separate, smaller list in `provision-workspace.mjs` than `SEEDED_FILES` (which covers `config/profile.yml`, `portals.yml`, etc.). `SEEDED_FILES` targets get ongoing additive-sync coverage via `backfill-templates.mjs` + `doctor-all.mjs`'s per-workspace health check, because they're expected to legitimately diverge (user personalization) while still needing new upstream fields synced in. `SYSTEM_FILE_COPIES` gets none of that — `backfill-templates.mjs` has zero awareness of `.claude/settings.json` (confirmed by grep). It's pure system/plumbing content, not personalization, yet nothing has ever watched it for drift. Separately, neither `doctor-all.mjs` nor `backfill-templates.mjs` is wired into `test-all.mjs` at all today, unlike `validate-script-references.mjs`, which runs automatically inside `test-all.mjs` (and therefore in CI on every PR) specifically so a class of past mistake (a moved script's stale references) can never silently ship again. That CI-enforced pattern — not "someone remembers to run a report" — is the bar this design holds itself to for `.claude/settings.json` drift.
...
This ships in the same change, not as a follow-up — it's what makes "did this reach every workspace" a CI question instead of a memory question, for this fix and every future one that touches a `SYSTEM_FILE_COPIES`-class file.

1. **Extend `backfill-templates.mjs` to track `.claude/settings.json`** as a new sync target alongside its existing `config/profile.yml`/`portals.yml` coverage. Unlike those two (YAML, comment-preserving), this is a JSON structural diff: for each top-level key present in the repo root's `.claude/settings.json` but absent (or missing specific entries — e.g. a `hooks.PreToolUse` array entry, an `permissions.allow` list item) from a workspace's copy, add it additively. Never remove a workspace-local addition that isn't in the root template — same non-destructive principle already governing the YAML side. `--check` reports drift without writing; `--apply` fixes it, exactly mirroring the existing profile/portals flow.
2. **Add a new automatic check to `core/test-all.mjs`**, styled directly on the existing `SYSTEM_PATHS coverage guard` / `validate-script-references.mjs` checks (both already run unconditionally inside `test-all.mjs`, which itself runs on every PR via GitHub Actions per `AGENTS.md`'s CI/CD section). Call it the **SYSTEM_FILE_COPIES drift guard**: it runs `backfill-templates.mjs --check --all` across every provisioned workspace and **fails the suite** if any workspace's `.claude/settings.json` is missing something the root template has. This is the mechanical enforcement — once it exists, a future PR that changes the root `.claude/settings.json` without also running the backfill fails CI automatically, the same hard guarantee `validate-script-references.mjs` already gives the "moved a script, forgot a reference" mistake.
3. **Add a short rule to `core/AGENTS.md`'s "Stack and Conventions" section**, next to the existing "Moving or renaming any script requires TWO reference sweeps" rule (same section, same pattern): *any change to a `SYSTEM_FILE_COPIES`-listed file requires re-running `backfill-templates.mjs --apply` (or a full `doctor-all.mjs` pass) across every provisioned workspace before the change ships — backstopped mechanically by `test-all.mjs`'s SYSTEM_FILE_COPIES drift guard, but do it anyway; the check should never be the first line of defense, only the one that can't be forgotten.* `AGENTS.md` is read automatically every session (unlike a dated spec file under `docs/superpowers/specs/`), which is what makes this instruction actually reach a future session instead of depending on it rediscovering this document.

Together, (2) is the guarantee — it does not depend on anyone, human or AI, remembering anything — and (3) is the same belt-and-suspenders convention this repo already applies to the structurally identical script-reference problem.
...
- **SYSTEM_FILE_COPIES drift guard test in `test-all.mjs`**: an integration test proving the guard actually fails when a workspace's `.claude/settings.json` is deliberately made to lag the root template (mirrors the existing `SYSTEM_PATHS coverage guard fails when it cannot inspect the tree` test pattern — the guard must fail loudly, not silently pass, when it can't do its job).
