...
2. Judge the loaded JD against those in a quick pass — do not skip reading the real JD text first.
   ...
   **Clearance gate specifically:** Run `node core/preflight-check.mjs --jd-file {{JD_FILE}}` — it searches the JD for clearance keywords and checks them against `config/profile.yml` → `clearance.status`/`clearance.accepts_sponsorship` deterministically, with zero tokens. (Company and role aren't known yet at this point in the run, so this is a gate-only call: the script skips its tracker dedup and reports `duplicate.checked: false`, which means "not attempted" — never read it as "not already tracked.") If `gate.pass` is `false` and `gate.reason` mentions clearance, this is a HARD STOP. If the JD mentions clearance but `gate.pass` is `true` (candidate accepts sponsorship), this is a soft gate — not a hard stop; proceed to full evaluation and note the requirement in Block A. If the script call itself fails (non-zero exit for any reason), fall back to applying the same clearance judgment yourself against the loaded JD and the profile's clearance posture, and continue — never skip the clearance gate because the script failed.
