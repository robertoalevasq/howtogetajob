...
// Task 1 move) — kept for sourcePath below, which reads the sibling
// update-system.mjs that moved into core/ alongside this file. REPO_ROOT is
// the actual repo root one level up: `git ls-files` must run there, not in
// core/, or it would only enumerate files under core/ (with core/-relative
// paths) instead of the whole tracked tree that SYSTEM_PATHS/USER_PATHS are
// defined against.
