...
| `apply-browser-holder.mjs` | Detached, per-{workspace, report} Playwright browser holder so a multi-turn Telegram application reconnects (via `@playwright/mcp --cdp-endpoint`) to the same already-authenticated browser instead of launching fresh every turn (`launchHolder`, run standalone with no args to stay alive as the holder itself). `--stop --report <num> --workspace <path>` is the actual cleanup command `modes/apply.md` Step 9 and its hard-stop branches run: force-kills the process (and Windows: its Chromium child tree) and removes the state entry directly, rather than sending a plain termination signal and trusting the holder's own handler to run — that assumption silently fails on Windows (this project's actual deployment platform), confirmed live across three real applications before `--stop` existed. 4-hour idle-lifetime backstop if every mode-file cleanup trigger is somehow missed |
...
| `preflight-check.mjs` | Zero-LLM pre-evaluation gate collapsing two previously-duplicated prompt-based pre-screens into one deterministic script: tracker dedup, a clearance / onsite-vs-`remote_only` keyword hard-stop against `config/profile.yml`, and advertised-comp extraction from JD text. Runs unconditionally (no provider config involved); `--company`/`--role` enable dedup, omitting them gives a gate-only run reporting `duplicate.checked: false`. Deliberately does NOT judge domain fit or archetype fit — too fuzzy for regex, still Claude's call |
| `ollama-delegate.mjs` | Opt-in single-task delegate for the three narrow, evidence-gathering sub-tasks a small model can do safely — `comp-market-estimate`, `block-g-signals`, `risk-summary-draft` (`node core/ollama-delegate.mjs <task> --input <file>`). Reads `config/llm-provider.yml` (absent = every call is a no-op), falls back Ollama Cloud → local Ollama (loopback-guarded) → non-zero exit so the caller does the task itself. Never produces a verdict: output is a pre-computed input Claude verifies and cites |
...
| `tune-targeting.mjs` | Zero-token `title_filter` retargeting analyzer — ranks candidate exclusion keywords and flags dead-weight positive keywords from a workspace's real scan corpus (`analyze`, JSON or `--summary`); on approval, retroactively prunes already-queued `data/pipeline.md` rows matching a finalized negative list (`prune --negative "..."`) since `title_filter` only applies at scan time. Never edits `portals.yml` itself — see `.claude/skills/run-tune-targeting/` |
...
| `admin-overview-snapshot.mjs` | Gathers a cross-workspace snapshot (roster, tracker stats, active-task status, token-usage/run-count history mined from Claude Code's own session transcripts) into one JSON file for the hub owner's admin overview — `node core/admin-overview-snapshot.mjs <output.json>` |
| `admin-overview-render.mjs` | Renders an admin-overview snapshot JSON into a static HTML fragment ready to publish as a Claude Artifact — `node core/admin-overview-render.mjs <snapshot.json> <output.html>` |
| `render-hub-control.mjs` | Builds a fresh snapshot (via `admin-overview-snapshot.mjs`'s `buildSnapshot()`) and injects it into `templates/hub-control-template.html`, producing the richer "Hub Control" admin dashboard — a second renderer alongside `admin-overview-render.mjs`'s plain fallback, driven by the `career-ops-dashboard` skill (`node core/render-hub-control.mjs [output.html]`, default `data/cache/hub-control.html`) |
| `token-efficiency-log.mjs` | Durable, transcript-derived per-session token log — every row is mined fresh from the same Claude Code session transcripts `admin-overview-snapshot.mjs` reads (never hand-maintained, never live-instrumented into any mode file), incrementally appended/upserted to `data/token-efficiency-log.tsv` via a per-file mtime cursor in `data/cache/token-efficiency-cursor.json` (`node core/token-efficiency-log.mjs update [--rebuild]`) |
| `token-efficiency-report.mjs` | Reads `token-efficiency-log.mjs`'s durable log (no transcript re-parsing) and reports totals by mode/workspace, the most expensive sessions, and efficiency flags (`high_snapshot_ratio`, `no_subagent_delegation`) for sessions worth a closer look — `node core/token-efficiency-report.mjs [--since Nd] [--workspace slug] [--summary]` |
...
| `doctor-all.mjs` | Cross-workspace health check — runs `doctor.mjs` plus `backfill-templates.mjs --check` against every provisioned workspace in one pass so a broken or out-of-date workspace is caught before its user hits it (JSON or plain-text summary); the `--check` pass covers not just `profile.yml`/`portals.yml` but also `.claude/settings.json` drift (a `SYSTEM_FILE_COPIES` entry — see `checkWorkspace`'s JSON pair support). Never applies a backfill fix itself (that stays a deliberate `backfill-templates.mjs --apply`), but inherits `doctor.mjs`'s own idempotent `_profile.md`/`_custom.md`/`_brief.md` template auto-copy, surfaced per workspace via `autoCopied` rather than done silently. Also runs one hub-global (not per-workspace) check: `checkTelegramWebhook()` flags a stray Telegram webhook, which blocks `getUpdates` polling — and therefore every workspace's Telegram commands — until cleared |
...
> - Remote only, or open to hybrid/onsite too?
> - Your salary target range
> - Do you need visa sponsorship to work there, or are you already authorized?
>
> I'll set everything up for you."

Fill `location.work_mode` (one of `remote_only`/`remote_preferred`/`hybrid_ok`/`onsite_ok`) and `location.visa_status`/`needs_sponsorship`/`authorized_in` from the actual answers — never leave the template's `"no_preference"`/`false`/`["United States"]` defaults in place unexamined once the user has responded, and never infer sponsorship status from location alone (living somewhere doesn't establish work authorization there). Both fields feed real evaluation scoring (`modes/oferta.md` Block A's Remote row and the work-authorization check), not just profile metadata.
...
**The example template's `tracked_companies` and `search_queries` are scaffolded around one demo archetype (AI/Forward-Deployed/DevRel-heavy, US+EU+global breadth) — most target roles won't match most of it.** `title_filter` alone doesn't fix this: a mismatched `tracked_companies`/`search_queries` entry still pays its real scan cost (a WebSearch call, or a Playwright/websearch company fetch) every run even when it can never pass the filter, which is exactly what makes a scan slow. After setting `title_filter.positive`, run every applicable step of **`modes/_portals-pruning.md`** — the same shared routine `telegram-onboarding` and `/settings` use — passing in the target roles, location, and (if industry-based) target industries from Step 2. It covers disabling mismatched `tracked_companies`/`search_queries` entries, writing an active `location_filter` (including always-allowing `"Remote"` when `work_mode` is `remote_only`/`remote_preferred`), and populating `industry_companies` for industry-based targeting — never delete entries, `enabled: false` keeps them available if targeting changes later.
...
Store insights in `config/profile.yml` (narrative), `_profile.md`, or `article-digest.md` (proof points) — never in `modes/_shared.md`. **Deal-breakers specifically also go into `_brief.md`'s "Hard DQ Criteria — instant FAIL" section** (one bullet per stated deal-breaker, in the user's own words) — that section is what `modes/triage.md` reads on every first-pass filtering decision, so a deal-breaker that only lives in `_profile.md`'s prose never actually short-circuits triage.
...
**After the user's first real scan**, if the results look off-target (too few hits, or matches that don't fit), mention `tune-targeting.mjs`: it ranks dead-weight `title_filter` keywords and flags mismatched scan queries using the real scan corpus as evidence, rather than guessing at a fix — see the `run-tune-targeting` skill. Onboarding itself never needs this (there's no scan history yet to analyze); it's the tool for after the fact.
...
## Rigorous Testing Before Declaring Done -- MANDATORY

A passing test suite proves the code you wrote does what you told it to do. It does not prove the code does what it needs to do — those are different claims, and the gap between them is exactly where a real bug hides. Match the verification method to what actually changed, and always construct inputs that look for room for error (boundary conditions, encoding edge cases, false-positive checks) — not just the happy path the change was written for.

- **Routing/dispatch behavior** (anything a Telegram message, `/command`, or headless dispatch reaches): verify with a real dispatch, not just a unit test against the prompt-building code. `core/telegram-emulate.mjs` drives real `claude -p` calls against synthetic messages in a disposable workspace for exactly this — see its own header comment for the pattern (`makeDisposableWorkspace()` + `dispatchOne()`/`createRoutingQueue()`). A mocked `invoke()` proves the plumbing is wired; it cannot catch a model-behavior bug, since the model's actual decision is exactly what a mock replaces.
- **Pure functions/logic** (a regex, a classifier, a formatter): unit tests are the right tool, but only if they include adversarial inputs, not just the case the fix was written for. Confirmed live 2026-09-14: a fix that made `application-answers.mjs` throw on a missing resume/CV filename used an ASCII-only regex (`/\b(resume|cv|curriculum vitae)\b/i`) that silently let an accented label ("Résumé") slip through untested — found only by deliberately testing an accented variant, not by the fix's own original test.
- **Security-relevant guards** (a `PreToolUse` hook, a permission gate): test the real CLI invocation (stdin in, exit code + stdout out) exactly as `.claude/settings.json` invokes it, not only the exported decision function. Confirmed live 2026-09-14: `guard-resume-filename.mjs`'s canonical-path regex required a leading path separator (`[\\/]output[\\/]...`), so a bare relative path like `path.join('output', filename)` produces (`output/021-x.pdf`, no leading separator) slipped through undetected — every existing test happened to use a path with a leading separator, so the gap was invisible until a deliberately bare-relative-path input was tried.
- **Real, multi-hour, or live-external-service flows** (a full `/run` cycle's ATS sweep, a live Playwright browser upload against a real ATS): a bounded/partial real test (e.g. a real dispatch with a short forced timeout, verifying only the reachable early steps) beats no real test, but say plainly what portion was actually exercised — do not claim full coverage for a check that only reached Pre-flight.

**When something you fixed doesn't fit any of the above cleanly** (pure documentation/prose changes, config-only changes), say so directly rather than picking the nearest test type and forcing it — a fix to a warning's wording doesn't need a dispatch, but it does need a `grep` confirming the exact text landed where intended.
...
- **RULE: `git clean`, `git rm --cached`, and orphan-branch experiments never run in the main working directory — always create an isolated worktree first (`git worktree add .worktrees/<name>`), even for a "quick" diagnostic test.** The main working directory routinely holds real, unrelated, uncommitted work sitting for days at a time — leaving edits uncommitted across sessions rather than committing/stashing promptly is a normal pattern here, not an edge case. Confirmed live 2026-09-14: diagnosing a `git push` failure, an orphan branch was created directly in the main working directory to build a minimal test pack; `git rm -rf --cached .` emptied the index, and the immediately-following `git clean -fd` then deleted every one of six unrelated modified files and three brand-new never-committed files, since all of them looked "untracked" once the index was empty. The three new files had zero git history to fall back on — full recovery was only possible because a stash commit from earlier in the same session happened to still be reachable in the object database after being dropped. Before ANY command that combines index manipulation (`rm --cached`, `reset --hard`, orphan checkout) with `git clean`, run `git status --porcelain -uall` first and treat anything it shows as load-bearing, not disposable scratch — then do the actual experiment in a worktree regardless of what that check finds.
