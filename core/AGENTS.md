...
| `preflight-check.mjs` | Zero-LLM pre-evaluation gate collapsing three previously-duplicated prompt-based pre-screens into one deterministic script: tracker dedup, a clearance / onsite-vs-`remote_only` keyword hard-stop against `config/profile.yml`, and advertised-comp extraction from JD text. Runs unconditionally (no provider config involved); `--company`/`--role` enable dedup, omitting them gives a gate-only run reporting `duplicate.checked: false`. Deliberately does NOT judge domain fit or archetype fit — too fuzzy for regex, still Claude's call |
