...
// the actual repo root that templates/ and output/ live under, and the base
// a user-supplied relative photo path resolves against (see
...
async function prepareCandidatePhoto(candidate) {
  ...
  const photoPath = isAbsolute(photo) ? photo : resolve(ROOT, photo);
...
async function main() {
  ...
  const [inputPath, outputPath, templateArg] = preview
    ? [args[1], resolve(ROOT, 'output', 'cv-preview.html'), args[2]]
    : args;
