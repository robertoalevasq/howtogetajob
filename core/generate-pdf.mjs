...
async function generatePDF() {
  ...
  // Anchored to the repo root (ROOT), not process.cwd(): running the script
  ...
  const relOut = relative(ROOT, outputPath);
  ...
  let cvMarkdown = '';
  try {
    cvMarkdown = await readFile(resolve(ROOT, 'cv.md'), 'utf-8');
  } catch (err) {
