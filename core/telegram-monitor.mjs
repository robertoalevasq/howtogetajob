...
4. Mark every finished row \`- [x]\` in \`data/pipeline.md\` with its disposition. A row you did not mark is a row that did not happen — the daemon counts \`- [ ]\` rows to decide whether to dispatch the next batch, so unmarked rows are the difference between progress and an infinite loop.
5. Results go to \`data/tracker-additions/{num}-{company-slug}.tsv\`, never a direct edit of \`data/applications.md\`.
6. Count EVERY row you finish (evaluated, pre-screened out, or errored). At 20, stop: write the checkpoint (\`step.id\` \`"2-pipeline"\`, top-level \`lastStopReason: "batch-limit"\`, \`counters.pipelineUrlsPending\` = rows still \`- [ ]\`), \`node core/cycle-lock.mjs release\`, summarise, end the turn. Do not continue to Step 3.
