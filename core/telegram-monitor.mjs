...
READ BUDGET — this is a resume, not a first run; orientation is the single largest avoidable cost here. Measured 2026-09-15 across eight real resume dispatches: ~96% of every session's tokens were cache_read (re-reading accumulated context once per turn, ~30 turns each), and one transcript read modes/cycle.md THREE times (567 lines each), re-read its own transcript, cat'd _custom.md, and listed core/ twice before touching a single URL. Every one of those turns permanently enlarges the context that all later turns pay to re-read, so a wasted read early costs far more than itself.
- Read each file you need exactly ONCE, and only these: \`data/pipeline.md\` (the backlog), \`data/cache/cycle-status.json\` (where the last batch stopped), \`cv.md\` + \`config/profile.yml\` + \`_profile.md\` (what you evaluate against), and \`modes/cycle.md\` Step 2 only if the summary below leaves you genuinely unsure.
- Do NOT \`ls\` to discover the layout (the known-paths block above already gives it), do NOT read your own session transcript, do NOT re-read a file you have already read this turn, and do NOT read \`modes/pipeline.md\` — its per-URL procedure is summarised below and agrees with \`modes/cycle.md\` Step 2.

Step 2, in short — the essentials, so you do not have to go re-derive them:
1. Process the \`- [ ]\` rows in \`data/pipeline.md\` inline, sequentially, in THIS turn. No \`Agent(...)\` fan-out: a headless run has no permission path for an unreviewed \`Agent\` call, and looping inline avoids re-paying the per-URL context-load cost a subagent dispatch incurs.
2. If more than ~20 rows are pending, pass \`--throttle\` to \`check-liveness.mjs\`.
3. Gate each posting through \`node core/preflight-check.mjs --company <c> --role <r> --jd-file <path>\` (it needs company+role for the dedup check, or at minimum \`--text\`/\`--jd-file\` for a gate-only run — never a bare URL) before spending any evaluation on it. It is zero-LLM: tracker dedup, clearance/onsite-vs-remote_only hard stops, advertised-comp extraction. A posting it hard-stops needs no evaluation from you at all.
4. Extracting a JD is the single most expensive thing you do per URL, so do it the cheap way. Check \`node core/jd-fetch-cache.mjs get "{url}"\` first — the liveness sweep already opened that exact page and persisted its text (2h TTL), so a hit means no second fetch at all. On a miss, if \`config/profile.yml\` sets \`scan.extractor: cli\`, run \`node core/browser-extract.mjs <url> --mode jd\` and use its \`text\`. Only if both are unavailable fall back to Playwright — and then **never a bare \`browser_snapshot\`**: pass \`target\` (the JD's main-content container) or a \`depth\` limit. Measured in this very pipeline on 2026-09-15: one 20-URL batch spent 23.30M tokens, 98.5% \`cache_read\`, on 21 full-page snapshots that every later turn then paid to re-read.
5. Mark every finished row \`- [x]\` in \`data/pipeline.md\` with its disposition. A row you did not mark is a row that did not happen — the daemon counts \`- [ ]\` rows to decide whether to dispatch the next batch, so unmarked rows are the difference between progress and an infinite loop.
6. Every posting you discard WITHOUT a full evaluation also gets one appended line in \`data/discard.log\`: \`{ISO8601}\\t{url}\\t{reason}\`, tab-separated, file created if absent. This is in addition to the \`- [x]\` row, not instead of it — the marked row tells the daemon the URL is done, the log is the auditable record of why, and it is what a later targeting review reads to tell "the filter is working" from "the filter is too aggressive". Confirmed live 2026-09-15: a batch that marked 15 discards correctly but skipped this log left no trace of its own reasoning. Read the \`{ISO8601}\` field from the real clock (\`date -u +%Y-%m-%dT%H:%M:%SZ\`) at the moment you append — never write a literal timestamp from memory or reuse one across rows. Confirmed live 2026-09-15: the very next batch wrote all 10 of its discard lines stamped \`20:15:00Z\` for work it did after \`20:46:08Z\`, making the column useless for the one question the log exists to answer — when targeting started rejecting a given kind of posting.
7. Results go to \`data/tracker-additions/{num}-{company-slug}.tsv\`, never a direct edit of \`data/applications.md\`.
8. Count EVERY row you finish (evaluated, pre-screened out, or errored). At 20, stop: write the checkpoint (\`step.id\` \`"2-pipeline"\`, top-level \`lastStopReason: "batch-limit"\`, \`counters.pipelineUrlsPending\` = rows still \`- [ ]\`), \`node core/cycle-lock.mjs release\`, summarise, end the turn. Do not continue to Step 3.

Never use AskUserQuestion, and never end this turn by asking whether to proceed — there is nobody to answer, and a question here strands the whole backlog until a human notices (confirmed live 2026-09-15). If a URL looks odd, process it on its merits or mark it skipped with a reason; either way keep going. Return a brief summary of what this batch did.`;
}
