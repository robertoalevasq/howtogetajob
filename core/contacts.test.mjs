...
// contacts.mjs resolves its paths from import.meta.url as ROOT = dirname of its
// own directory (it now lives one level down, in core/ — #workspace-multitenancy
// Task 1), so a copy of the script into temp/core/ makes the temp dir a fully
// isolated career-ops root: data/contacts.tsv and output/ directly under the
// temp dir, no dependence on whatever the caller's real workspace contains — a
// contributor with a real phonebook gets the same results as CI.
...
const tmpScript = join(tmpRoot, 'core', 'contacts.mjs');
try {
...
try {
  ...
  const emptyVcfOut = execFileSync('node', [join(emptyRoot, 'core', 'contacts.mjs'), '--vcf'], { encoding: 'utf-8', timeout: 10000 });
