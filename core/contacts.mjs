...
// the actual repo root that data/ and output/ live under, and the anchor the
// path-traversal guards below need (see #workspace-multitenancy Task 1).
const ROOT = dirname(CAREER_OPS);
const CONTACTS_PATH = join(ROOT, 'data/contacts.tsv');
const DEFAULT_VCF = join(ROOT, 'output/contacts.vcf');
...
function writeVcf(contacts, quality) {
  ...
  // repo. Anchored to the repo root (ROOT), not process.cwd() — see the
