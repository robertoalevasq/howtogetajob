...
// core/, so the syntax-checked set now lives one directory below ROOT.
const mjsFiles = readdirSync(join(ROOT, 'core')).filter(f => f.endsWith('.mjs'));
...
const syntaxWorker = async () => {
  for (let i = nextSyntaxIdx++; i < mjsFiles.length; i = nextSyntaxIdx++) {
    try {
      await execFileAsync(NODE, ['--check', mjsFiles[i]], { cwd: join(ROOT, 'core'), timeout: 30000 });
      syntaxOk[i] = true;
    } catch (err) {
      syntaxOk[i] = false;
...
try {
  ...
  for (const { name, allowFail } of scripts) {
    ...
    // #workspace-multitenancy Task 1 moved every root-level *.mjs script into
    // core/, so the copy created by copyDirSync() below has each script one
    // directory deeper than its bare name in `scripts` above suggests.
    const result = run(NODE, [join(scriptTmp, 'core', scriptFile), ...args], {
      cwd: scriptTmp,
...
try {
  const { classifyLiveness } = await import(pathToFileURL(join(ROOT, 'core', 'liveness-core.mjs')).href);
  ...
  const { resolveAtsApi, classifyAshbyBoard, checkLivenessViaApi } = await import(pathToFileURL(join(ROOT, 'core', 'liveness-api.mjs')).href);
  ...
  const { checkUrlLiveness, checkUrlLivenessWithFallback, isChallengeResult, jitteredDelayMs } =
    await import(pathToFileURL(join(ROOT, 'core', 'liveness-browser.mjs')).href);
  ...
  const { rejectPrivateOrInvalid, setHostResolver } = await import(
    pathToFileURL(join(ROOT, 'core', 'liveness-browser.mjs')).href
  );
  const blockCases = [
...
const stagedBlob = (path) => {
  const entry = run('git', ['ls-files', '-s', path], { cwd: ROOT });
...
// the guard must exit non-zero.
{
  ...
  try {
    // Mirror the real core/ nesting (#workspace-multitenancy Task 1): the
    // coverage guard now resolves its own directory as ROOT (for its sibling
    // update-system.mjs) and that directory's PARENT as the `git ls-files`
    // cwd. A flat copy directly into probeDir would make the parent of the
    // copy's own directory the *real* repo tree (full of tracked files),
    // defeating this probe's whole point — the copy must sit one level deep,
    // exactly like core/, so probeDir itself (untracked, nothing below it)
    // is what `git ls-files` sees and correctly returns empty.
    const probeCoreDir = join(probeDir, 'core');
    mkdirSync(probeCoreDir, { recursive: true });
    copyFileSync(join(ROOT, 'core', 'validate-system-paths-coverage.mjs'), join(probeCoreDir, 'validate-system-paths-coverage.mjs'));
    copyFileSync(join(ROOT, 'core', 'update-system.mjs'), join(probeCoreDir, 'update-system.mjs'));
    const probe = spawnSync(process.execPath, [join(probeCoreDir, 'validate-system-paths-coverage.mjs')], {
      cwd: probeCoreDir,
      encoding: 'utf-8',
    });
    if (probe.status !== 0) {
...
{
  const cov = spawnSync(process.execPath, [join(ROOT, 'core', 'validate-system-paths-coverage.mjs')], {
    cwd: ROOT,
    encoding: 'utf-8',
  });
  if (cov.status === 0) {
...
{
  const untrusted = spawnSync(process.execPath, [join(ROOT, 'core', 'validate-untrusted-content-coverage.mjs')], {
    cwd: ROOT,
    encoding: 'utf-8',
  });
...
const generatePdfScript = readFile('core/generate-pdf.mjs');
...
try {
  const { validateCvSectionOrder } = await import(pathToFileURL(join(ROOT, 'core', 'generate-pdf.mjs')).href);
...
try {
  const { repoRelativeManifestPath, injectPrintPageCss } = await import(pathToFileURL(join(ROOT, 'core', 'generate-pdf.mjs')).href);
...
try {
  const { renderHtmlToPdf } = await import(pathToFileURL(join(ROOT, 'core', 'generate-pdf.mjs')).href);
...
const updateSystemScript = readFile('core/update-system.mjs');
if (
...
try {
  const {
    ...
    parsePositiveInt,
    reexecTimeoutMs,
  } = await import(pathToFileURL(join(ROOT, 'core', 'update-system.mjs')).href);
...
const patternsMachineFields = readFile('core/analyze-patterns.mjs').match(/const MACHINE_SUMMARY_FIELDS = new Set\(\[([\s\S]*?)\]\);/)?.[1] ?? '';
if (
...
let pdfModule = null;
try {
  pdfModule = await import(pathToFileURL(join(ROOT, 'core', 'generate-pdf.mjs')).href);
} catch (e) {
...
const updaterSrc = readFile('core/update-system.mjs');
if (
...
const scanScript = readFile('core/scan.mjs');
if (
...
try {
  const { formatPipelineOffer, formatCompensation } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  const { appendToPipeline } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  const { appendToPipeline } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
  const { acquirePipelineLock, LockTimeoutError } = await import(pathToFileURL(join(ROOT, 'core', 'pipeline-lock.mjs')).href);
...
try {
  ...
  try {
    ...
    process.chdir(fixtureRoot);
    const { loadSeenUrls } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  const { parseBlacklist, loadBlacklist } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
  ...
  const { normalizeCompany } = await import(pathToFileURL(join(ROOT, 'core', 'tracker-utils.mjs')).href);
...
try {
  ...
  const okEntry = localParser.detect({
    ...
    parser: { command: 'node', script: 'core/scan.mjs' },
  });
  ...
  if (localParser.detect({ name: 'Yahoo!', careers_url: 'https://x.co', parser: { command: 'node', script: 'core/scan.mjs' } })?.url) {
...
try {
  const { entryOnHost } = await import(pathToFileURL(join(ROOT, 'core', 'scan-ats-full.mjs')).href);
...
try {
  const { classifyPostingDate, sampleCompanies } = await import(pathToFileURL(join(ROOT, 'core', 'scan-ats-full.mjs')).href);
...
try {
  const { filterBlacklistedOffers } = await import(pathToFileURL(join(ROOT, 'core', 'scan-ats-full.mjs')).href);
...
try {
  const { passesFilters } = await import(pathToFileURL(join(ROOT, 'core', 'scan-ats-full.mjs')).href);
  const { buildTitleFilter, buildLocationFilter, buildContentFilter } =
    await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  const { removeRowByNum } = await import(pathToFileURL(join(ROOT, 'core', 'tracker.mjs')).href);
  const md = [
    '# Applications',
    '',
    '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |',
...
try {
  const nodeTrackerWriters = [
    ['core/dedup-tracker.mjs', 1],
    ['core/normalize-statuses.mjs', 1],
    ['core/reply-watch.mjs', 1],
    ['core/tracker.mjs', 2],
  ];
...
`, 'utf-8');
  ...
  const exampleResult = run(NODE, [join(ROOT, 'core', 'validate-portals.mjs'), '--file', 'templates/portals.example.yml'], { cwd: ROOT });
...
try {
  const { deriveSlugCandidates, parseAtsSlug, verifyCompanies, classifyFetchError } =
    await import(pathToFileURL(join(ROOT, 'core', 'verify-portals.mjs')).href);
...
try {
  const { splitCompanyBlocks, computeFixes } = await import(
    pathToFileURL(join(ROOT, 'core', 'fix-slugs.mjs')).href
  );
  ...
  try {
    ...
    // nothing but 15 s.
    execFileSync(NODE, [join(ROOT, 'core', 'fix-slugs.mjs'), '--file', dryRunPortals, '--dry-run'], {
      cwd: ROOT,
      timeout: 2000,
    });
  } catch {
...

{
  ...
  try {
    const updater = await import(pathToFileURL(join(ROOT, 'core', 'update-system.mjs')).href);
    const sample = [
    ...
    const liveSource = readFileSync(join(ROOT, 'core', 'update-system.mjs'), 'utf-8');
...

{
  ...
  try {
    const updaterSource = readFileSync(join(ROOT, 'core', 'update-system.mjs'), 'utf-8');
...
try {
  ...
  // Both CLIs now share parseSinceDays, so the flag cannot mean two things.
  {
    ...
    const atsSrc = readFileSync(join(ROOT, 'core', 'scan-ats-full.mjs'), 'utf-8');
  ...
  {
    const sinceCli = (...argv) => spawnSync(NODE, [join(ROOT, 'core', 'scan.mjs'), ...argv], {
      cwd: ROOT,
      encoding: 'utf-8',
      timeout: 30000,
      stdio: ['pipe', 'pipe', 'pipe'],
  ...
  const { matchedTitleKeywords } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
console.log('\n11b. Title filter — acronym word boundaries');
try {
  const { buildTitleFilter, compileKeyword } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  ...
  let cliOut = '';
  try {
    cadence = await import(pathToFileURL(join(ROOT, 'core', 'followup-cadence.mjs')).href);
    ...
    try {
      cliOut = execFileSync(NODE, [join(ROOT, 'core', 'followup-cadence.mjs')], {
        cwd: ROOT,
        encoding: 'utf-8',
        timeout: 30000,
  ...
  // is where a silently-inferred age would actually do damage.
  {
    ...
    try {
      // Mirror the real core/ nesting (#workspace-multitenancy Task 1):
      // followup-cadence.mjs and tracker-parse.mjs resolve tracker-aliases.json
      // and data/applications.md via import.meta.url one directory above their
      // own location, so a flat copy into e2eTmp directly would look for both
      // one level too high (outside e2eTmp entirely).
      const e2eCore = join(e2eTmp, 'core');
      ...
      // Node's module resolution walks upward from the importing file, so a
      // node_modules symlink at e2eTmp (one level above e2eCore) is still
      // found by the copy running from e2eCore.
      symlinkSync(
      ...
      const e2eOut = execFileSync(NODE, [join(e2eCore, 'followup-cadence.mjs')], {
        cwd: e2eTmp,
        encoding: 'utf-8',
        timeout: 30000,
...
try {
  const addMod = await import(pathToFileURL(join(ROOT, 'core', 'add-entry.mjs')).href);
  ...
  try {
    ...
    execFileSync(NODE, [join(ROOT, 'core', 'add-entry.mjs'), payloadPath, '--dry-run'], { env, encoding: 'utf-8' });
    ...
    const realOut = JSON.parse(execFileSync(NODE, [join(ROOT, 'core', 'add-entry.mjs'), payloadPath], { env, encoding: 'utf-8' }));
    ...
    const rerun = JSON.parse(execFileSync(NODE, [join(ROOT, 'core', 'add-entry.mjs'), payloadPath], { env, encoding: 'utf-8' }));
...
try {
  const { normalizeReportLink } = await import(pathToFileURL(join(ROOT, 'core', 'tracker-links.mjs')).href);
  const repo = '/repo';
  ...
  const { resolveReportPath } = await import(pathToFileURL(join(ROOT, 'core', 'followup-cadence.mjs')).href);
...
try {
  const RESERVE = join(ROOT, 'core', 'reserve-report-num.mjs');
  ...
  const trackerParseApi = await import(pathToFileURL(join(ROOT, 'core', 'tracker-parse.mjs')).href);
  ...
  const evaluatorSources = ['core/ollama-eval.mjs', 'core/openai-eval.mjs', 'core/gemini-eval.mjs', 'core/openrouter-runner.mjs']
...
try {
  ...
  try {
    ...
    let dupNumOut;
    try {
      dupNumOut = execFileSync(NODE, [join(ROOT, 'core', 'verify-pipeline.mjs')], { cwd: ROOT, env: dupNumEnv, encoding: 'utf-8', timeout: 30000, stdio: ['pipe', 'pipe', 'pipe'] });
...
try {
  const { roleFuzzyMatch, roleTokens } = await import(pathToFileURL(join(ROOT, 'core', 'role-matcher.mjs')).href);
...
try {
  const { rebuildRow } = await import(pathToFileURL(join(ROOT, 'core', 'tracker-utils.mjs')).href);
...
try {
  const { resolveColumns, parseTrackerRow, LEGACY_COLMAP } = await import(pathToFileURL(join(ROOT, 'core', 'tracker-parse.mjs')).href);
...
try {
  const { parseTrackerRows, parsePdfIndex, findMatches } = await import(pathToFileURL(join(ROOT, 'core', 'find.mjs')).href);
...
try {
  const { roleFuzzyMatch } = await import(pathToFileURL(join(ROOT, 'core', 'role-matcher.mjs')).href);
...
try {
  const { resolveScoreStatus, looksLikeScoreCell } = await import(pathToFileURL(join(ROOT, 'core', 'tracker-parse.mjs')).href);
...
try {
  ...
  try {
    ...
    const collisionResult = spawnSync(NODE, [join(ROOT, 'core', 'merge-tracker.mjs')], {
      cwd: ROOT,
      encoding: 'utf-8',
      env: { ...process.env, CAREER_OPS_TRACKER: reservedTracker, CAREER_OPS_ADDITIONS: reservedAdditions },
    });
...
try {
  ...
  try {
    ...
    let badCode = 0;
    try {
      badOut = execFileSync(NODE, [join(ROOT, 'core', 'verify-pipeline.mjs')], {
    ...
    let hdrCode = 0;
    try {
      hdrOut = execFileSync(NODE, [join(ROOT, 'core', 'verify-pipeline.mjs')], {
        cwd: ROOT, encoding: 'utf-8', timeout: 30000,
    ...
    const trackerParse = await import(pathToFileURL(join(ROOT, 'core', 'tracker-parse.mjs')).href);
  ...
  while (retries >= 0) {
    ...
    function spawnMerge(additionsDir, holdMs = 0) {
      ...
      const result = new Promise(resolve => {
        const child = spawn(NODE, [join(ROOT, 'core', 'merge-tracker.mjs')], {
          cwd: ROOT,
          env: {
            ...process.env,
...
if (!sqliteAvailable) {
  ...
  try {
    ...
    try {
      ...
      // tracker.mjs reads templates/states.yml via a bare repo-root-relative
      // path (not env-overridable, and not resolved from import.meta.url like
      // MD_PATH's default), so it must run with cwd: ROOT rather than run()'s
      // core/-default cwd — matching how a real invocation (npm run tracker,
      // cwd = the package.json directory) always sees it.
      const trackerRun = (args) => run(NODE, [join(ROOT, 'core', 'tracker.mjs'), ...args], { cwd: ROOT, env, stdio: ['pipe', 'pipe', 'pipe'] });
...
try {
  const doctorScript = readFile('core/doctor.mjs');
  if (
...
try {
  const { extractCareersUrlDomain, pickRediscoveredUrl } = await import(
    pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href
  );
...
try {
  ...
  const { SEMVER_RE } = await import(pathToFileURL(join(ROOT, 'core', 'update-system.mjs')).href);
...
try {
  const { buildHtml } = await import(pathToFileURL(join(ROOT, 'core', 'generate-cover-letter.mjs')).href);
...
try {
  ...
  const usageOut = execFileSync(process.execPath, [join(ROOT, 'core', 'generate-cover-letter.mjs'), '--help'], { encoding: 'utf-8' });
...
try {
  ...
  const { inlineLocalFonts } = await import(pathToFileURL(join(ROOT, 'core', 'generate-pdf.mjs')).href);
...
function latexValidate(tex) {
  ...
  let out;
  try {
    out = execFileSync(NODE, [join(ROOT, 'core', 'generate-latex.mjs'), texPath], { cwd: ROOT, encoding: 'utf-8', timeout: 30000 });
  } catch (e) {
...
try {
  ...
  const { validateLatexContent } = await import(pathToFileURL(join(ROOT, 'core', 'generate-latex.mjs')).href);
  ...
  execFileSync(NODE, [join(ROOT, 'core', 'extract-latex-content.mjs'), join(ROOT, 'examples/latex-tex/resume-subheading.tex'), '--out', extractOut], { cwd: ROOT, encoding: 'utf-8' });
  ...
  execFileSync(NODE, [join(ROOT, 'core', 'patch-latex-content.mjs'), join(ROOT, 'examples/latex-tex/resume-subheading.tex'), patchJson, patchedTex], { cwd: ROOT, encoding: 'utf-8' });
...
try {
  ...
  const updater = readFileSync(join(ROOT, 'core', 'update-system.mjs'), 'utf-8');
...
try {
  const { parsePortals } = await import(pathToFileURL(join(ROOT, 'core', 'openrouter-runner.mjs')).href);
...
console.log('\n44b. openrouter-runner — prompt-cache breakpoint (#1709)');
try {
  const { buildCachedSystemMessage } = await import(pathToFileURL(join(ROOT, 'core', 'openrouter-runner.mjs')).href);
...
console.log('\n44c. openai-eval — host-gated prompt-cache breakpoint (#1709)');
try {
  const src = readFileSync(join(ROOT, 'core', 'openai-eval.mjs'), 'utf-8');
  const checks = [
...
console.log('\n44d. gemini-eval — static prefix as systemInstruction (#1709)');
try {
  const src = readFileSync(join(ROOT, 'core', 'gemini-eval.mjs'), 'utf-8');
...
console.log('\n44f. openai-tailor — host-gated prompt-cache breakpoint (#1709, #2432)');
try {
  const src = readFileSync(join(ROOT, 'core', 'openai-tailor.mjs'), 'utf-8');
  const checks = [
    // api.openai.com gets a plain-string system message (auto-caches; may reject the field)
...
try {
  const src = readFileSync(join(ROOT, 'core', 'ollama-eval.mjs'), 'utf-8');
...
console.log('\n45. Scan cooldown filter');
try {
  const { addDays, buildCooldownFilter, shouldDedupScanHistoryRow } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  ...
  const vreg = await import(pathToFileURL(join(ROOT, 'core', 'validate-plugin-registry.mjs')).href);
  const audit = await import(pathToFileURL(join(ROOT, 'core', 'plugin-audit.mjs')).href);
  const install = await import(pathToFileURL(join(ROOT, 'core', 'plugin-install.mjs')).href);
  ...
  const scan = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
  ...
  if (ids === 'apify,discord,gmail,notion,telegram') pass('all 5 bundled reference plugins discovered (apify, discord, gmail, notion, telegram)');
  else fail(`bundled plugins = "${ids}" (expected apify,discord,gmail,notion,telegram)`);
  ...
  // Updater registration (SYSTEM vs USER split). plugins.mjs is shipped via the
  // single 'core/' directory entry now (#workspace-multitenancy Task 1), not a
...
// Real session files contain real names/companies — they MUST be gitignored.
{
  ...
  if (run('git', ['check-ignore', real], { cwd: ROOT })) {
...
try {
  ...
  const { parseStories, tokenize, score } = await import(pathToFileURL(join(ROOT, 'core', 'match-star.mjs')).href);
...
`.trim();
  ...
  if (existsSync(join(ROOT, 'core', 'match-star.mjs'))) {
...
try {
  const src = readFile('core/prepare-application.mjs');
...
try {
  ...
  const trackerSrc = readFileSync(join(ROOT, 'core', 'tracker.mjs'), 'utf-8');
  ...
  const scanSrc = readFileSync(join(ROOT, 'core', 'scan.mjs'), 'utf-8');
  ...
  {
    const coreCad = readFileSync(join(ROOT, 'core', 'followup-cadence.mjs'), 'utf-8')
  ...
  {
    const corePrereqBlock = readFileSync(join(ROOT, 'core', 'doctor.mjs'), 'utf-8')
...
try {
  const { fingerprintText, similarity, findCrossListings, normalizeJdText, FINGERPRINT_MIN_TEXT } =
    await import(pathToFileURL(join(ROOT, 'core', 'fingerprint-core.mjs')).href);
...
try {
  const { formatScanHistoryRow } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
{
  const unit = run(NODE, ['--test', 'test/cv-templates.test.mjs'], { cwd: ROOT });
...
{
  const unit = run(NODE, ['--test', 'test/pipeline-lock.test.mjs'], { cwd: ROOT });
...
{
  const unit = run(NODE, ['--test', 'test/cover-resolver.test.mjs'], { cwd: ROOT });
...
try {
  const src = readFileSync(join(ROOT, 'core', 'test-all.mjs'), 'utf-8');
  ...
  const { appendScanRunSummary, SCAN_RUNS_HEADER } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
  ...
  const { computeConsecutiveFailures } = await import(pathToFileURL(join(ROOT, 'core', 'scan.mjs')).href);
...
try {
  const freshnessSrc = readFile('core/check-table-freshness.mjs');

  const updaterSrc = readFile('core/update-system.mjs');
  const freshSysBlock = (updaterSrc.match(/SYSTEM_PATHS\s*=\s*\[([\s\S]*?)\]/) || [, ''])[1];
  // #workspace-multitenancy Task 1: core/*.mjs scripts (including
  // check-table-freshness.mjs) are shipped via the single 'core/' directory
  // entry now, not a per-file bare-filename entry.
  if (freshSysBlock.includes("'check-table-freshness.mjs'") || freshSysBlock.includes("'core/'")) {
  ...
  if (pkg.scripts && pkg.scripts.freshness === 'node core/check-table-freshness.mjs') {
