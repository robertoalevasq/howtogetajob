...
// Task 1 move) — used below to address sibling scripts spawned by runScript().
// REPO_ROOT is the actual repo root one level up: the cwd those children need,
// since several of them (tracker.mjs's STATES_PATH, e.g.) fall back to a bare
// cwd-relative path like 'templates/states.yml' when no env override is set.
...
const HAS_WEB = existsSync(join(REPO_ROOT, 'web', 'src', 'lib', 'tracker-table.mjs'));
...
`;
  const rows = parseApplications(WEB_10COL, REPO_ROOT);
  const r = rows[0];
  ...
  const webAliases = loadHeaderAliases(REPO_ROOT);
